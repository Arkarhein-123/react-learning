...
import { useAllGetEnrolledCourses } from "../../course/query/CourseHookQuery.tsx";

// Defining the localized interface properly to match standard expectations
...
export default function EnrolledCourseComponent() {
  const { data: enrolledCourses, isLoading } = useAllGetEnrolledCourses();
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">
        <span className="text-fuchsia-900 capalize italic ">{username}</span>{" "}
        Enrolled Courses
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {enrolledCourses?.map((course: Course) => (
          <div
            key={course.courseId} // Updated to courseId
          ...
          >
            ...
            <div className="px-5 pb-5 pt-0 bg-white">
              <Link
                to={`/enrolled-courses/${course.courseId}/lessons`}
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2.5 px-4 rounded-lg shadow-sm shadow-indigo-100 transition-colors duration-200 flex items-center justify-center gap-2 group"
              >
                <span>Learn</span>
                <svg
