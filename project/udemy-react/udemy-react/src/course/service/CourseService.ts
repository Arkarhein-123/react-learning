...
export const getLessonsRequest = async (id: number): Promise<Lesson[]> => {
  const response = await axios.get(`${COURSE_URL}/${id}/course-lessons`);
  return response.data;
};

export const addLessonsApiRequest = async (id: number, lesson: Lesson) => {
  const response = await axios.post(`${COURSE_URL}/${id}/lessons`, lesson);
  return response.data;
};

export const enrolledCourseRequest = async (
  courseIds: EnrolledDto,
): Promise<string> => {
  const response = await axios.post(`${COURSE_URL}/enroll`, courseIds);
  return response.data;
};
export const getEnrolledCourseRequest = async (): Promise<Course[]> => {
