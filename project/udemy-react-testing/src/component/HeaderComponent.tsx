...
import { isLoggedIn, logout, getRoleName } from '../auth/service/AuthService';

export default function HeaderComponent() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const beLoggedIn = isLoggedIn();
  const isTeacher = beLoggedIn && getRoleName() === 'ROLE_TEACHER';
  const navigator = useNavigate();
  const [searchParams] = useSearchParams();
  const [searchValue, setSearchValue] = useState(searchParams.get('search') || '');

  // Keep search input in sync when URL parameter changes externally
  useEffect(() => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchValue.trim()) {
      navigator(`/?search=${encodeURIComponent(searchValue.trim())}`);
    } else {
      navigator('/');
    }
  };
  ...
  return (
    <nav className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50 text-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center gap-4">
          ...
          {/* Search Bar */}
          <form onSubmit={handleSearchSubmit} className="flex-1 max-w-2xl hidden sm:block text-black">
            <div className="relative">
              ...
              <button type="submit" className="absolute inset-y-0 left-0 pl-3 flex items-center cursor-pointer">
                <svg className="h-5 w-5 text-gray-400 hover:text-indigo-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="hidden md:flex items-center gap-4">
            <Link to="/teacher" className="text-sm font-medium text-gray-700 hover:text-indigo-600 transition-colors">
              {isTeacher ? "Instructor Dashboard" : "Teach on Udemy"}
            </Link>
           ...
           
            {
              beLoggedIn &&
                 ...
                 className="px-4 py-2 text-sm font-medium text-indigo-600 bg-white border border-indigo-600 rounded-md hover:bg-indigo-50 transition-all duration-200 cursor-pointer">
              Logout
            </button>
            }
           {
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 pt-2 pb-4 space-y-3 shadow-inner">
          ...
          {beLoggedIn && (
            <button onClick={logoutHandler} className="block w-full text-center px-4 py-2 text-sm font-medium text-indigo-600 border border-indigo-600 rounded-md hover:bg-indigo-50">
              Logout
            </button>
          )}
          <hr className="border-gray-200" />
          <Link to="/teacher" className="block text-sm font-medium text-gray-700 hover:text-indigo-600 py-1">
            {isTeacher ? "Instructor Dashboard" : "Teach on Udemy"}
          </Link>
