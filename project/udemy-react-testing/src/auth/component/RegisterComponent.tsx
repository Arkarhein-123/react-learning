...
export default function RegisterComponent() {
  const { register, handleSubmit, setValue, watch } = useForm<RegisterDto>({
    defaultValues: {
      userType: "STUDENT",
  ...
  // Watch fields to eliminate local state conflicts
  const userType = watch("userType");
  const isTeacher = userType === "TEACHER";
  const skillsValue = (watch("skills") as unknown as string) || "";

  // Shared Tailwind classes
  const labelClasses = "block text-sm font-medium text-slate-700 mb-1";
  const inputClasses =
    "w-full px-4 py-2 bg-white border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all duration-200";
      ...
      const payload: RegisterDto = {
        ...
        skills:
          ...
          skillsValue.trim() !== ""
            ...
            : [],
        education: isTeacher ? data.education || "" : "",
        address: !isTeacher ? data.address || "" : "",
        studentEducation: !isTeacher
          ? data.studentEducation || "UNDER_GRADUATE"
          : "",
      };

      await registerApiCall(payload);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-xl shadow-slate-200/50 rounded-2xl sm:px-10 border border-slate-100">
          {message && (
            <div className="mb-4 p-3 bg-emerald-50 text-emerald-700 rounded-lg text-sm border border-emerald-200 animate-fadeIn">
              {message}
            </div>
          )}
          {error && (
            <div className="mb-4 p-3 bg-rose-50 text-rose-700 rounded-lg text-sm border border-rose-200 animate-fadeIn">
              {error}
            </div>
          )}
